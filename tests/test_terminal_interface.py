...
def run_script_with_ars(args: list) -> (str, int):
    """
    :return: The terminal output created by the scrupt
    """
...
class TerminalInterfaceTest(unittest.TestCase):
    ...
    fpga_in_boundaries_pll_f = [(fpga.get_f_in_min("mmcm"), fpga.get_f_in_max("mmcm")) for fpga in fpgas]


    def setUp(self) -> None:
        self.args = ["python", "main.py"]

    def test_bool_args(self):
        for arg in self.bool_args:

            output, code = run_script_with_ars(self.args + [arg])
            self.assertEqual(code, 0)

    def test_frequency_boundaries_pll(self):
        for fpga in self.fpga_specifications:
            for in_boundaries in self.fpga_in_boundaries_pll_f:
                for out_boundaries in self.fpga_out_boundaries_pll_f:
                    args_combinations = [
                        [
                    ...
                    for args in args_combinations:
                        s, code = run_script_with_ars(self.args + args)
                        self.assertEqual(code, 0)

    def test_frequency_boundaries_mmcm(self):
        for fpga in self.fpga_specifications:
            for in_boundaries in self.fpga_in_boundaries_mmcm_f:
                for out_boundaries in self.fpga_out_boundaries_mmcm_f:
                    args_combinations = [
                        [
